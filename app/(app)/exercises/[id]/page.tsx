...
export default function ExerciseDetail() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async ({ signal }) => {
      const res = await fetch('/api/exercises?id=' + id, { cache: 'no-store', signal });
    ...
    return (
      <section className="py-6 md:py-8 pb-28 md:pb-16">
        ...
        <div className={`${cardBase} mt-4`}>
          <h2 className="font-manrope text-lg md:text-xl font-semibold text-v-dark dark:text-white">
            Couldn’t load exercise
          </h2>
          <p className="mt-2 text-sm text-v-dark/75 dark:text-white/75">{(error as Error)?.message ?? 'Unknown error'}</p>
          <button
            onClick={() => refetch()}
            className="mt-3 inline-flex items-center gap-2 rounded-xl bg-v-ceil px-4 py-2 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-v-ceil/30"
          >
            Retry
          </button>
        </div>
    ...
    <section className="py-6 md:py-8 pb-28 md:pb-16">
      ...
      <header className="mt-3 grid items-start gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {(data.durationLabel || data.duration) && (
              <span className={chipNeutral}>{data.durationLabel ?? `${data.duration}m`}</span>
            )}
          </div>
        </div>
        ...
        <div className="md:justify-self-end md:self-start">
          <Link
            href={`/exercises/${data.id}?tab=tutorial`}
...
function TimerSession({ ex }: { ex: Exercise }) {
  ...
  return (
    <div className="mt-4 flex flex-1 flex-col">
      {/* DURATION: individual chip buttons (no long pill container) */}
...
function DetailSkeleton() {
  return (
    <section className="py-6 md:py-8 pb-28 md:pb-16">
      <div className="h-4 w-24 rounded bg-v-dark/10 dark:bg-white/10" />
      <div className="mt-3 grid items-start gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
        <div>
          <div className="h-8 w-56 rounded bg-v-dark/10 dark:bg-white/10" />
          <div className="mt-2 h-4 w-3/4 rounded bg-v-dark/10 dark:bg-white/10" />
          <div className="mt-2 flex gap-2">
            <div className="h-6 w-20 rounded-full bg-v-dark/10 dark:bg-white/10" />
            <div className="h-6 w-20 rounded-full bg-v-dark/10 dark:bg-white/10" />
            <div className="h-6 w-12 rounded-full bg-v-dark/10 dark:bg-white/10" />
          </div>
        </div>
        <div className="h-10 w-44 rounded-xl bg-v-dark/10 dark:bg-white/10" />
      </div>

      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_380px]">
        <div className="space-y-5">
          <div className="h-48 rounded-2xl border border-v-dark/10 bg-v-white dark:bg-[#0b1a33] dark:border-white/10" />
          <div className="h-32 rounded-2xl border border-v-dark/10 bg-v-white dark:bg-[#0b1a33] dark:border-white/10" />
        </div>
        <div className="h-64 rounded-2xl border border-v-dark/10 bg-v-white dark:bg-[#0b1a33] dark:border-white/10" />
      </div>
    </section>
  );
}
